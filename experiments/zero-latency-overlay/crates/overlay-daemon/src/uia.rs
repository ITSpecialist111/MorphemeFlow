...
pub fn detect_visible_text_regions(max_regions: usize) -> Result<Vec<SemanticRegion>> {
    let (_, regions) = detect_visible_text_regions_with_window(max_regions)?;
    Ok(regions)
}

#[cfg(target_os = "windows")]
pub fn detect_visible_text_regions_with_window(max_regions: usize) -> Result<(isize, Vec<SemanticRegion>)> {
    use windows::Win32::Foundation::{HWND, RPC_E_CHANGED_MODE};
    use windows::Win32::System::Com::{CoCreateInstance, CoInitializeEx, CLSCTX_INPROC_SERVER, COINIT_MULTITHREADED};
    use windows::Win32::UI::Accessibility::{
        CUIAutomation, IUIAutomation, IUIAutomationTextPattern, IUIAutomationValuePattern, TreeScope_Subtree,
        UIA_DocumentControlTypeId, UIA_EditControlTypeId, UIA_HyperlinkControlTypeId, UIA_TextControlTypeId,
    ...
    for i in 0..count {
        ...
        if text.is_empty() {
                ...
                unsafe { element.GetCurrentPatternAs::<IUIAutomationTextPattern>(UIA_TextPatternId) }
            {
                ...
                if text.is_empty() {
                    if let Ok(ranges) = unsafe { pattern.GetVisibleRanges() } {
                        let count = unsafe { ranges.Length().unwrap_or(0) };
                        if count > 0 {
                            if let Ok(first_range) = unsafe { ranges.GetElement(0) } {
                                text = unsafe { first_range.GetText(220) }
                                    .map(|value| value.to_string())
                                    .unwrap_or_default()
                                    .trim()
                                    .to_string();
    ...
    Ok((window_id, post_process_regions(regions, max_regions)))
}
...
#[cfg(not(target_os = "windows"))]
pub fn detect_visible_text_regions_with_window(_max_regions: usize) -> Result<(isize, Vec<SemanticRegion>), String> {
    Ok((0, Vec::new()))
}
...
fn merge_same_line(regions: Vec<SemanticRegion>) -> Vec<SemanticRegion> {
    ...
    for region in regions {
        if let Some(last) = merged.last_mut() {
            let same_line = (region.rect.top - last.rect.top).abs() <= 12
                && (region.rect.bottom - last.rect.bottom).abs() <= 18;
            let gap = region.rect.left - last.rect.right;
            let adjacent = (-6..=90).contains(&gap);
            let merged_text_len = last.text.chars().count() + region.text.chars().count();

            if same_line && adjacent && merged_text_len <= 420 {
